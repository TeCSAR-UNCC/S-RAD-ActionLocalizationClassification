...
def construct_loader(cfg,dataset = None,num_segments = 8,batch_size = 3,
    ...
    if dataset == 'virat':
        ...
        data_loader = torch.utils.data.DataLoader(Ava(cfg,split = split,
               num_segments=num_segments,dense_sample = cfg.DENSE_SAMPLE,
               uniform_sample=cfg.UNIFORM_SAMPLE,random_sample = cfg.RANDOM_SAMPLE,
               strided_sample = cfg.STRIDED_SAMPLE),batch_size=batch_size,
               shuffle = shuffle,num_workers=cfg.NUM_WORKERS,pin_memory=True,
               drop_last = drop_last) #change shuffle to true
        return data_loader
