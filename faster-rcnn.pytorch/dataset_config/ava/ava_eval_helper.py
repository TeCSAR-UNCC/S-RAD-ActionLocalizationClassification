...
from dataset_config.ava.ava_evaluation import (
...
def read_csv(csv_file, class_whitelist=None, load_score=False, num_seg=8):
    ...
    Returns:
      boxes: A dictionary mapping each unique image key (string) to a list of
        boxes, given as coordinates [y1, x1, y2, x2]. [x1, y1, x2,y2]
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            if load_score:
                score = float(row[7])
            boxes[image_key].append([y1, x1, y2, x2]) #changes here
...
def evaluate_ava(
    ...
    write_results(detections, "detections_%s.csv" % name,log)
    write_results(groundtruth, "groundtruth_%s.csv" % name,log)

    results = run_evaluation(categories, groundtruth, detections, excluded_keys,log)

    info = ("AVA eval done in %f seconds." % (time.time() - eval_start))
...
def run_evaluation(
    ...
    for image_key in boxes:
        #key_im = image_key.split(',')[0]+","+image_key.split(',')[1] #remove
        if image_key in excluded_keys:
            logging.info(
                (
...
def get_ava_eval_data(
   ...
   # boxes,
    ...
    for i in range(len(scores)):
        ...
        for det in one_scores:
         cls_score = det[5:].tolist() #assume 5 is backgrnd class
         x1,y1,x2,y2 = det[:4]
         box = [y1,x1,y2,x2]
         for cls_idx,score in enumerate(cls_score):
...
def write_results(detections, filename,log):
    ...
    '''info = ("AVA results wrote to %s" % filename
            "\ttook %d seconds." % (time.time() - start))
    log.write(info + '\n')
    log.flush()''' 
